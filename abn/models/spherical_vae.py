...
class SphericalVAE(torch.nn.Module):
    ...
    def log_likelihood(self, x, n=10):
        ...
        log_p_x_z = -torch.nn.BCEWithLogitsLoss(reduction='none')(x_mb_, x.reshape(-1, self.input_dim).repeat((n, 1, 1))).sum(-1)
        ...
        return ((log_p_x_z + log_p_z - log_q_z_x).t().logsumexp(-1) - torch.log(torch.tensor(n))).mean()