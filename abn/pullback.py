...
def neural_immersion(theta, model):
    """ Computes the immersion from S^1 [position of rat along circle] to R^N, 
    the place cell neural state space.
    ...
    x_mu, x_logvar = recon_x
    return x_mu


def get_metric(intrinsic_dim, immersion, model_filename):
    ...
    metric = PullbackMetric(dim=intrinsic_dim, embedding_dim=data_dim, immersion = immersion)
