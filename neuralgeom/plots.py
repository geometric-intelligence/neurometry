...
def create_plots(
    ...
    if config.dataset_name == "s1_synthetic":
        fig_curv, fig_curv_analytic = plot_curv(
            angles, mean_curvature_norms, mean_curvature_norms_analytic, config
        )
...
def plot_curv2(angles, mean_curvature_norms, mean_curvature_norms_analytic, config):
    ...
    if config.dataset_name == "s1_synthetic":
        ...
        plt.savefig(
            f"results/figures/{config.results_prefix}_curv_profile_analytic.png"
        )
        plt.savefig(
            f"results/figures/{config.results_prefix}_curv_profile_analytic.svg"
        )
    elif config.dataset_name == "s2_synthetic":
        fig_analytic = plt.figure(figsize=(40, 20))
        color_norm = mpl.colors.Normalize(0.0, 1.2 * max(mean_curvature_norms_analytic))
    ...
    if config.dataset_name in ("experimental", "s1_synthetic"):
        ...
        y = config.radius * [np.sin(angle[0]) * np.sin(angle[1]) for angle in angles]
        z = config.radius * [np.cos(angle[0]) for angle in angles]
        sc = ax_learned.scatter3D(
            x, y, z, s=30, c=mean_curvature_norms, cmap="Spectral", norm=color_norm
        )
        ax_learned.set_title(
            "Norm of mean curvature obtained through learned immersion"
        )
        plt.savefig(f"results/figures/{config.results_prefix}_curv_profile_learned.png")
        plt.savefig(f"results/figures/{config.results_prefix}_curv_profile_learned.svg")
        plt.colorbar(sc)
