...
class TrainDataset:
    ...
    def _generate_dx_list(self, max_dr, interval=1.0):
        dx_list = []
...
class EvalDataset:
    ...
    def __iter__(self):
        while True:
            yield {
                "traj_vis": self._gen_trajectory_vis(
                    self.config.n_traj_vis, self.config.n_inte_step_vis
                ),
                "traj": self._gen_trajectory(
                    self.config.n_traj, self.config.n_inte_step
                ),
            }
