...
def construct_block_diagonal_weights(
    ...
    if not antisym:
        M = tf.Variable(
            tf.random_normal(
                [num_block, num_channel, block_size, block_size], stddev=0.001
            ),
            name=name,
        )
    else:
        M = tf.Variable(
            tf.random_normal(
                [num_block, num_channel, int(block_size * (block_size + 1) / 2)],
                stddev=0.001,
            ),
            name=name,
        )
...
def draw_path_to_target(
    ...
    if isinstance(place_seq,list) or np.ndim(place_seq) > 2:
        ...
        if isinstance(place_seq,list):
            place_seq = place_seq[0]
        plt.plot(
            place_seq[:, 1],
            place_len - place_seq[:, 0],
            "o-",
            ms=6,
            lw=2.0,
            color="darkcyan",
...
def draw_path_integral(place_len, place_seq, col=(255, 0, 0),target=None):
    ...
    plt.imshow(
        np.swapaxes(canvas, 0, 1), interpolation="nearest", cmap=cmap, aspect="auto"
    )
    return canvas
...
def create_custom_writer(
    logdir: str, process_index: int = 0, asynchronous=True
) -> MetricWriter:
...
def draw_path_to_target_planning(
    place_len,
    place_seq,
    scale,
    save_file=None,
    ...
    def plot_by_scale(s, label):
        ...
        plt.scatter(
            place_seq[idx, 1], place_len - place_seq[idx, 0], label=label, color=color
        )
        for id in idx:
            plt.plot(
                place_seq[(id - 1, id), 1],
                place_len - place_seq[(id - 1, id), 0],
                color=color,
            )
...
def visualize_u(model, sess, test_dir, epoch=0, final_epoch=False, result_dir=None):
    ...
    for i in range(len(weights_u_value_transform)):
        ...
        draw_heatmap_2D(weight_to_draw)
    plt.savefig(
        os.path.join(test_dir, "weightsu_%04d.png" % epoch), bbox_inches="tight"
    )
    if final_epoch:
        if not os.path.exists(result_dir):
            os.makedirs(result_dir)
