...
def load_activations(epochs,version="single",verbose=True):
    ...
    random.seed(0)
    for epoch in epochs:
        activations_epoch_path = activations_dir + f"activations_{version}_agent_epoch_{epoch}.npy"
        rate_map_epoch_path = activations_dir + f"rate_map_{version}_agent_epoch_{epoch}.npy"
        if os.path.exists(activations_epoch_path) and os.path.exists(rate_map_epoch_path):
