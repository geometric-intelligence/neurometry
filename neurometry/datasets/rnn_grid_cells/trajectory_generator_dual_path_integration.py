...
class TrajectoryGenerator:
    ...
    def get_generator(self, batch_size=None, box_width=None, box_height=None):
        ...
        while True:
            ...
            v = np.stack(
                [
                    traj_self["ego_v"] * np.cos(traj_self["target_hd"]),
                    traj_self["ego_v"] * np.sin(traj_self["target_hd"]),
                    traj_other["ego_v"] * np.cos(traj_other["target_hd"]),
                    traj_other["ego_v"] * np.sin(traj_other["target_hd"]),
                ],
                axis=-1,
            )
            ...
            place_outputs_self[
                :, :, torch.arange(0, place_outputs_self.size()[2], 2)
            ] = 0
            place_outputs_other = self.place_cells.get_activation(pos[:, :, 2:])
            place_outputs_other[
                :, :, torch.arange(1, place_outputs_other.size()[2], 2)
            ] = 0
            place_outputs = (
                place_outputs_self + place_outputs_other
            )  # np.max(np.stack([place_outputs_self,  place_outputs_other], axis = -1), axis = -1) #
            ...
            init_actv_self = self.place_cells.get_activation(
                init_pos[:, :, :2]
            ).squeeze()
            init_actv_other = self.place_cells.get_activation(
                init_pos[:, :, 2:]
            ).squeeze()
    ...
    def get_test_batch(self, batch_size=None, box_width=None, box_height=None):
        ...
        place_outputs_self = self.place_cells.get_activation(pos[:, :, :2])
        place_outputs_self[:, :, torch.arange(0, place_outputs_self.size()[2], 2)] = 0
        place_outputs_other = self.place_cells.get_activation(pos[:, :, 2:])
        place_outputs_other[:, :, torch.arange(1, place_outputs_other.size()[2], 2)] = 0
        place_outputs = (
            place_outputs_self + place_outputs_other
        )  # np.max(np.stack([place_outputs_self,  place_outputs_other], axis = -1), axis = -1) #
        place_outputs = torch.tensor(place_outputs, dtype=torch.float32)
    ...
    def get_test_batch_single_agent(
        self, batch_size=None, box_width=None, box_height=None
    ):
        """For testing performance, returns a batch of smample trajectories"""
        if not batch_size:
            batch_size = self.options.batch_size
        ...
        pos = np.stack(
            [
                traj_self["target_x"],
                traj_self["target_y"],
                np.zeros(np.shape(traj_self["target_x"])),
                np.zeros(np.shape(traj_self["target_y"])),
            ],
            axis=-1,
        )
        pos = torch.tensor(pos, dtype=torch.float32).transpose(0, 1)
        ...
        init_pos = np.stack(
            [
                traj_self["init_x"],
                traj_self["init_y"],
                np.zeros(np.shape(traj_self["init_x"])),
                np.zeros(np.shape(traj_self["init_y"])),
            ],
            axis=-1,
        )
        init_pos = torch.tensor(init_pos, dtype=torch.float32)
