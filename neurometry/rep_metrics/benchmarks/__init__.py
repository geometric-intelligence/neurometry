...
def average_rdms(rdms):
    return 1 - fisherz_inv(
        fisherz(np.stack([1 - rdm for rdm in rdms]))
...
class NSDBenchmark:
    def __init__(
        ...
        clean_rdms_only=True,
        anatomical_roi_subset=None,
        functional_roi_subset=None,
    ):
        print(
            "Now loading the {} image set and the {} voxel set...".format(
                image_set, voxel_set
            )
        )
        ...
        self.stimulus_data["image_path"] = (
            self.image_root + "/" + self.stimulus_data.image_name
        )
        ...
        self.anatomical_rois = (
            anatomical_rois if not anatomical_roi_subset else functional_roi_subset
        )
        self.functional_rois = (
            functional_rois if not functional_roi_subset else anatomical_roi_subset
        )

        self.all_rois = [
            roi
            for roi in self.metadata.columns
            if roi in self.anatomical_rois + self.functional_rois
        ]
    ...
    def get_rdms(self, roi_subset=None, include_group_average=False):
        ...
        if include_group_average:
            for roi in brain_rdms:
                brain_rdms[roi]["group_average"] = average_rdms(
                    brain_rdms[roi].values()
                )
